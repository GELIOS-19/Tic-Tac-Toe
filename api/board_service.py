...
class BoardService:
    def __init__(self, board_side_length=3):
        ...
        self.board = Board(cells=[Cell(index=i, player=Player(Player.NULL_PLAYER), filled=False) for i in range(self.total_squares)],
                           current_player=Player(Player.X_PLAYER))
    ...
    def check_winner(self, player, round_count):
        for winning_position_set in self.winning_position_sets:
            ...
            conditions = [self.board.cells[winning_position_set[i]].player.mark == player.mark
                          for i in range(self.board_side_length)]
            true_count = 0
            for condition in conditions:
                if condition: true_count += 1
            # set completed status and who the winner is
            if true_count == self.board_side_length: return {"completed": True, "winner": player}
        # check for draw
        if round_count > self.total_squares - 1: return {"completed": True, "winner": Player(Player.NULL_PLAYER)}
