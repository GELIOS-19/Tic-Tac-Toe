...
class GameService:
    ...
    def player_turn(self, index: int, current_player: Player):
        if not self.board_service.board.cells[index].filled:
            ...
            # check winners
            # x
            check = self.board_service.check_winner(
                player=Player(Player.X_PLAYER), round_count=self.round_count
            )
            ...
            # o
            check = self.board_service.check_winner(
                player=Player(Player.O_PLAYER), round_count=self.round_count
            )
            self.completed = check["completed"]
            self.winner = check["winner"]
            if self.completed:
